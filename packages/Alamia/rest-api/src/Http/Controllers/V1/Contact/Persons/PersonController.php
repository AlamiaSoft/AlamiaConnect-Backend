...
namespace Alamia\RestApi\Http\Controllers\V1\Contact\Persons;

use Illuminate\Http\JsonResponse;
use Illuminate\Http\Resources\Json\JsonResource;
use Illuminate\Support\Facades\Event;
use Prettus\Repository\Criteria\RequestCriteria;
use Alamia\Admin\Http\Requests\AttributeForm;
use Webkul\Contact\Repositories\PersonRepository;
use Alamia\RestApi\Http\Controllers\V1\Controller;
use Alamia\RestApi\Http\Request\MassDestroyRequest;
use Alamia\RestApi\Http\Resources\V1\Contact\PersonResource;
use Webkul\User\Repositories\UserRepository;

use Alamia\RestApi\Http\Criteria\PersonSearchCriteria;
...
class PersonController extends Controller
{
    ...
    public function search(): JsonResource
    {
        if ($userIds = $this->getAuthorizedUserIds()) {
            $persons = $this->personRepository
                ->pushCriteria(new PersonSearchCriteria())
    ...
    private function sanitizeRequestedPersonData(): array
    {
        ...
        if (isset($data['emails'])) {
            $data['emails'] = collect($data['emails'])->filter(fn ($email) => ! is_null($email['value']))->toArray();
        }
