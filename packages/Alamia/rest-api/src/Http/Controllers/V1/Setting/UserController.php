...
use Alamia\Admin\Notifications\User\Create;
use Alamia\RestApi\Http\Controllers\V1\Controller;
use Alamia\RestApi\Http\Request\MassDestroyRequest;
use Alamia\RestApi\Http\Request\MassUpdateRequest;
...
class UserController extends Controller
{
    ...
    public function store(): JsonResource
    {
        ...
        return (new UserResource($admin))
            ->additional(['meta' => ['message' => trans('rest-api::app.settings.users.create-success')]]);
    }
    ...
    public function update($id)
    {
        ...
        return (new UserResource($admin))
            ->additional(['meta' => ['message' => trans('rest-api::app.settings.users.updated-success')]]);
    }
    ...
    {
        if (auth()->guard()->user()->id == $id) {
            return response()->json($this->jsonApiError('Delete Failed', trans('rest-api::app.settings.users.delete-failed'), 400), 400);
        } elseif ($this->userRepository->count() == 1) {
            return response()->json($this->jsonApiError('Delete Failed', trans('rest-api::app.settings.users.last-delete-error'), 400), 400);
        } else {
            ...
            try {
                ...
                return response()->json($this->jsonApiMessage(trans('rest-api::app.settings.users.delete-success')));
    ...
    public function massUpdate(MassUpdateRequest $massUpdateRequest): JsonResource
    {
        ...
        if (! $count) {
            return response()->json($this->jsonApiError('Update Failed', trans('rest-api::app.settings.users.mass-update-failed'), 500), 500);
        }

        return response()->json($this->jsonApiMessage(trans('rest-api::app.settings.users.mass-update-success')));
    }
    ...
    public function massDestroy(MassDestroyRequest $massDestroyRequest): JsonResource
    {
        ...
        if (! $count) {
            return response()->json($this->jsonApiError('Delete Failed', trans('rest-api::app.settings.users.mass-delete-failed'), 500), 500);
        }

        return response()->json($this->jsonApiMessage(trans('rest-api::app.settings.users.mass-delete-success')));
    }
}
