...
use Alamia\Admin\Http\Requests\LeadForm;
...
use Alamia\RestApi\Http\Resources\V1\Lead\LeadResource;
use Alamia\RestApi\Http\Resources\V1\Product\LeadProductResource;
use Alamia\RestApi\Http\Resources\V1\Setting\StageResource;
...
class LeadController extends Controller
{
    ...
    {
        $query = $this->leadRepository->query();
        ...
        if ($search = request('search')) {
            $query->where(function($q) use ($search) {
                $q->where('leads.title', 'like', '%' . $search . '%')
                  ->orWhere('leads.description', 'like', '%' . $search . '%')
        ...
        foreach (request()->except(array_merge($this->excludeKeys, ['include', 'counts'])) as $input => $value) {
        ...
        $leads = $query->paginate(request()->input('limit') ?? 10);
    ...
    public function store(LeadForm $request): JsonResource
    {
        ...
        return (new LeadResource($lead))
            ->additional(['meta' => ['message' => trans('rest-api::app.leads.create-success')]]);
    }
    ...
    public function update(LeadForm $request, int $id): JsonResource
    {
        ...
        return (new LeadResource($lead))
            ->additional(['meta' => ['message' => trans('rest-api::app.leads.updated-success')]]);
    }
    ...
    public function addProduct(int $leadId): JsonResource
    {
        ...
        return (new LeadProductResource($product))
             ->additional(['meta' => ['message' => trans('rest-api::app.leads.updated-success')]]);
    }
    ...
    public function removeProduct(int $id)
    {
        try {
            ...
            return response()->json($this->jsonApiMessage(trans('rest-api::app.leads.delete-success')));
        } catch (\Exception $exception) {
            return response()->json($this->jsonApiError('Delete Failed', trans('rest-api::app.leads.delete-failed'), 500), 500);
        }
    }
    ...
    public function massUpdate(MassUpdateRequest $massUpdateRequest)
    {
        ...
        return response()->json($this->jsonApiMessage(trans('rest-api::app.leads.updated-success')));
    }
