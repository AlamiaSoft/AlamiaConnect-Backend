...
class Installer extends Command
{
     ...
     * @var string
     */
    protected $description = 'AlamiaConnect installer.';
    ...
    public function handle()
    {
        ...
        // Clean up paths and find all files except problematic ones
        $filesToRun = [];
        ...
        $this->info('Running ' . count($filesToRun) . ' migrations...');
        foreach ($filesToRun as $f) {
            if (strpos($f, '2021_09_30') !== false) {
                $this->line('Will run: ' . $f);
            }
        }
        ...
        // Now run the shim migration in a second step to handle SQLite compatibility
        $this->call('migrate', [
        ...
        $this->warn('Step: Seeding basic data for AlamiaConnect kickstart...');
    ...
    protected function createAdminCredentials()
    {
        ...
        try {
            ...
            $this->info('The installation has been finished and you can now use AlamiaConnect.');
